...
### GitHub で Windows EXE（ZIP）をダウンロード

`main` 向けの CI ビルドが完了すると、次の **Releases** に **`NPU_Audio_Enhancer_windows.zip`** が添付されます（タグ `windows-exe` はビルドのたびに上書きされます）。

| 説明 | リンク |
|------|--------|
| リリース一覧・説明・履歴 | **[Releases](https://github.com/mj2cvqj7ct-creator/NPU-AI/releases)** |
| ローリング版（`windows-exe`）のページ | **[Release: windows-exe](https://github.com/mj2cvqj7ct-creator/NPU-AI/releases/tag/windows-exe)** |
| **ZIP を直接ダウンロード** | **[NPU_Audio_Enhancer_windows.zip](https://github.com/mj2cvqj7ct-creator/NPU-AI/releases/download/windows-exe/NPU_Audio_Enhancer_windows.zip)** |

ZIP を展開し、フォルダ内の **`NPU_Audio_Enhancer.exe`** を実行してください。404 の場合は、下記の Actions でビルドがまだ成功していないか、Release が未作成です。
