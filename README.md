...
- `Ctrl+1-6` - タブ切替（Effects/DAC/Recommend/Stats/Perf/Debug）
- `Ctrl+Up/Down` - マスターボリューム調整
- `Ctrl+Shift+A` - プリセット比較モード
- `F1` - チュートリアル/ヘルプ表示
...
- 8バンドパラメトリックEQ周波数応答カーブを対数周波数グリッドに表示
- Enhancement パラメータ変更時にリアルタイム更新
- バンドマーカー、ベルカーブ、グラデーション塗りつぶし

### Audio Player & Transport Bar
- WAVファイルのオフライン再生（DSP処理パイプライン経由）
- 再生/一時停止/停止/シーク/ループ制御バー
- タイム表示、プログレスバー、ファイル情報

### Export Queue（バッチエクスポート）
- 複数ファイルの連続バッチエクスポート
- バックグラウンドDSP処理＋進捗追跡
- キャンセル、完了後キュークリア対応

### Session History（セッション履歴）
- プリセット変更、ジャンル検出、ファイル入出力のイベントログ
- JSON永続化（`~/.npu_audio_enhancer/session_history.json`）
- セッション統計（処理フレーム数、A/B比較回数等）

### Spectrogram Display
- リアルタイムスクロールスペクトログラム（時間-周波数ヒートマップ）
- Viridis風カラーマップ（512点FFT）
- 周波数ラベル表示

### NPU Performance Monitor
- 処理時間リアルタイムグラフ（ラインチャート）
- 平均/ピーク処理時間、バジェット使用率、FPS表示
- NPU Active/Off ステータス
- ベンチマーク機能

### Preset Comparison Mode
- 2つのプリセットを同時にセット＆クロスフェード比較
- `Ctrl+Shift+A`で切替
- スムーズ遷移（50フレームクロスフェード）

### Hotkey Customization
- 全キーボードショートカットをカスタマイズ可能
- `~/.npu_audio_enhancer/hotkeys.json`に永続化
- コンフリクト検出、デフォルトリセット

### Multi-Format Export
- WAV / FLAC / OGG 出力対応（soundfile統合）
- FLAC圧縮レベル、OGG品質設定
- scipy fallback（soundfile未インストール時）

### User Profiles
- 複数ユーザープロファイル保存/切替
- プリセット/EQパラメータ/出力設定をプロファイル毎に管理
- `~/.npu_audio_enhancer/profiles/`に永続化

### Clipping Indicator
- リアルタイムピーク/True Peak（4xオーバーサンプリング）表示
- 0 dBFS超過でCLIP LED赤点灯＋クリップカウント
- 自動リセット（3秒後）

### Preset Import/Export
- プリセットをJSONファイルとして共有（.npu_preset / .npu_presets）
- 単体＆パック（複数プリセット一括）エクスポート
- バリデーション＆バージョン互換チェック

### Audio Device Selector
- 入力/出力デバイス選択UI（DACタブ内）
- サンプルレート/チャンネル数表示
- SABAJ A20D排他モード対応

### Tutorial / Help Overlay
- `F1`キーでチュートリアルオーバーレイ表示
- 4ステップガイド（概要/エフェクトチェーン/プリセット/ショートカット）
- 前へ/次へナビゲーション
...
### Unit Tests
- 200テスト（プリセット / プロセッサー / エフェクト / NPU / A/B比較 / 設定 / ファイルI/O / プロファイラー / プレイヤー / セッション履歴 / エクスポートキュー / EQ / スペクトログラム / プリセット比較 / ホットキー / フォーマット変換 / プロファイル / プリセットI/O / クリッピング）
