...
class DACControlPanel(QGroupBox):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self._exclusive_check.setToolTip(
            "WASAPI 排他モードでビットパーフェクトに近い出力を行います",
        )
        ...
        btn_row = QHBoxLayout()
        self._optimize_btn = QPushButton("NPU 最適化")
        ...
        self._optimize_btn.setToolTip(
            "NPU 処理に合わせてバッファとレイテンシを自動調整します",
        )

        self._resync_loopback_btn = QPushButton("ループバックを再同期")
        ...
        self._resync_loopback_btn.setToolTip(
            "Windows の既定再生ミックスを再取得して同期します。"
            "処理中はループバックを再起動、停止中はレート表示のみ更新します。"
    ...
    def update_status(self, status_info: dict[str, Any]) -> None:
        ...
        name = str(status_info.get("device_name", "不明"))
